...
import { Select, Spinner, Textarea, useToast } from "@chakra-ui/react";
...
const VisitRegistration = () => {
  ...
  const [isDoctorSpecialitiesLoading, setIsDoctorSpecialitiesLoading] =
    useState(true);
  ...
  async function getDoctorSpecialities() {
    ...
    const specialties = await fetch(
      `${process.env.REACT_APP_BACKEND_HOST}/api/doctor/specialities`,
      {
        mode: "cors",
        credentials: "include",
        headers: {
  ...
  async function getListOfClinics() {
    ...
    const clinics = await fetch(
      `${process.env.REACT_APP_BACKEND_HOST}/api/clinic`,
      {
        mode: "cors",
        credentials: "include",
        headers: {
  ...
  async function getDoctorsForClinic(clinicId) {
    ...
    const doctors = await fetch(
      `${process.env.REACT_APP_BACKEND_HOST}/api/clinic/${clinicId}/doctor`,
      {
        method: "GET",
        mode: "cors",
        credentials: "include",
  ...
  async function getDoctorsForSpeciality(speciality) {
    setIsDoctorListLoading(true);
    const doctors = await fetch(
      `${process.env.REACT_APP_BACKEND_HOST}/api/doctor/specialities/${speciality}`,
      {
        method: "GET",
        mode: "cors",
        credentials: "include",
  ...
  async function postVisit() {
    const visitData = await fetch(
      `${process.env.REACT_APP_BACKEND_HOST}/api/visit`,
      {
        method: "POST",
  ...
  useEffect(() => {
    getDoctorsForClinic(visitDetails.clinic.clinicId)
      .then((doctors) => {
        if (doctors) {
          setDoctorList(() => {
            return doctors;
  ...
  useEffect(() => {
    ...
    getDoctorsForSpeciality(visitDetails.doctorSpeciality)
      .then((doctors) => {
        if (doctors) {
          setDoctorList(() => {
            return doctors;
  ...
  const submitVisit = (e) => {
    ...
    const form = document.getElementById(
      visitDetails.online ? "visit-form-online" : "visit-form-clinic"
    );
  ...
  function visitForm(online) {
    return (
      <form
        id={online ? "visit-form-online" : "visit-form-clinic"}
      ...
      >
        ...
        <div className="form-group info-border">
          ...
          <Textarea
            ...
            rows="5"
            onChange={changeVisitDescription}
          ></Textarea>
        </div>
        <button
  ...
  return (
    <>
      <div
        className={
          "container col-6 mx-auto m-3 rounded-5 bg-light text-dark bg-opacity-1 green-shadow"
        }
      >
        <h1 className={"fs-1 pt-3 text-center"}>Make an appointment</h1>

        <div className={"container mx-auto m-3 px-5 py-3"}>
          <label className={"p-3 fs-5"} htmlFor={"specialities"}>
            What kind of service do you require?:
          </label>
          <Select
            name={"specialities"}
            className={"form-select"}
            onChange={changeDoctorSpeciality}
            required
          >
            <option value="" hidden>
              - Select a Speciality -
        ...
        <nav className={"d-flex justify-content-center align-items-center"}>
          <div className={"nav nav-tabs nav-fill"} id="nav-tab" role="tablist">
            ...
            <button
              ...
              data-bs-toggle={"tab"}
              id={"nav-clinic-tab"}
              data-bs-target={"#nav-clinic"}
              type={"button"}
              role={"tab"}
              aria-controls={"nav-clinic"}
              aria-selected={"false"}
              onClick={toggleOnline}
            >
              Meet in person
