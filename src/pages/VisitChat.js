...
const getVisit = async (visitId) => {
  const data = await fetch(
    `${process.env.REACT_APP_BACKEND_HOST}/api/visit/${visitId}`,
    {
      credentials: "include",
    }
  );
...
export default function VisitChat(props) {
  ...
  const setStatusVisit = async () => {
    fetch(
      `${process.env.REACT_APP_BACKEND_HOST}/api/visit/status/${props.match.params.visitId}`,
      {
        credentials: "include",
        method: "PUT",
        headers: {
          ...
          <div className="col-8">
            <div className=" mx-auto rounded-5 bg-dark text-dark bg-opacity-10 shadow mt-3">
              ...
              <form
                ...
                ref={formRef}
                className="new-chat px-4 py-4 | my-5 pb-3"
              >
                <div className="input-group">
                  <div className="input-group-prepend">
                    <span className="message__label | input-group-text">
                      Your message:
                    </span>
                  </div>
                  <input
                  ...
                  <div className="input-group-append">
                    <button
                      type="submit"
                      className="btn__chat | btn btn-primary"
                    >
                      Send
                    </button>
                  </div>
